...
config = RobertaConfig.from_pretrained(
    "transformers/PhoBERT_base_transformers/config.json",
    from_tf=False,
    num_labels=NUMLABLES,
...
def convert_data_to_df(json_data):
    ...
    return pd.DataFrame(
        list(zip(tag, context, linking, question)),
        columns=["tag", "context", "linking", "question"],
    )
...
class DuckBot:
    def __init__(self):
        ...
        self.tokenizer_question_answering = AutoTokenizer.from_pretrained(
            model_checkpoint
        )
        self.model_question_anwering = MRCQuestionAnswering.from_pretrained(
            model_checkpoint
        )

        # SETTUP MODEL TEXT CLASSIFICATION
        self.model_text_classification = (
            RobertaForSequenceClassification.from_pretrained(
                "transformers/PhoBERT_base_transformers/model.bin", config=config
            )
        )
        self.model_text_classification.load_state_dict(model_state)

        self.rdrsegmenter = VnCoreNLP(
            "transformers/vncorenlp/VnCoreNLP-1.1.1.jar",
            annotators="wseg",
            max_heap_size="-Xmx500m",
        )
        self.tokenizer_text_classification = AutoTokenizer.from_pretrained(
    ...
    def get_tag(self, question):
        ...
        with torch.no_grad():
            outputs = self.model_text_classification(
    ...
    def run(self, question, last_tag=None):
        ...
        if tag == None:
            ...
            img_text, suggest_text = self.create_more_info(tag)
            print(f"suggest text:{suggest_text}")
